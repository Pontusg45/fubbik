...
export const fileRefRoutes = new Elysia()
    .get("/chunks/:id/file-refs", ctx =>
        Effect.runPromise(requireSession(ctx).pipe(Effect.flatMap(session => fileRefService.getFileRefs(ctx.params.id, session.user.id))))
    )
    .put(
        "/chunks/:id/file-refs",
        ctx =>
            Effect.runPromise(
                requireSession(ctx).pipe(Effect.flatMap(session => fileRefService.setFileRefs(ctx.params.id, session.user.id, ctx.body)))
            ),
        {
            body: t.Array(
                t.Object({
    ...
    .get("/file-refs", ctx =>
        Effect.runPromise(requireSession(ctx).pipe(Effect.flatMap(session => fileRefService.listAll(session.user.id))))
    )
    .get(
        "/file-refs/lookup",
        ctx =>
