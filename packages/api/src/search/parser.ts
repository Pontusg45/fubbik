 ...
 */
function resolveOperatorAndValue(field: string, rawValue: string): { operator: string; value: string; params?: Record<string, string> } {
    // connections:3+  → gte
...
export function parseQueryString(input: string): QueryClause[] {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (colonIdx > 0) {
            ...
            const { operator, value, params } = resolveOperatorAndValue(field, rawValue);
    ...
    return clauses
        .map(clause => {
            ...
            if (clause.field === "text") {
                const v = clause.value.includes(" ") ? `"${clause.value}"` : clause.value;
                return `${prefix}${v}`;
            }

            if (clause.field === "path" && clause.params?.from && clause.params?.to) {
                const from = clause.params.from.includes(" ") ? `"${clause.params.from}"` : clause.params.from;
                const to = clause.params.to.includes(" ") ? `"${clause.params.to}"` : clause.params.to;
