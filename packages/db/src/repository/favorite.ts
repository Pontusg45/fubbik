...
export function listFavorites(userId: string) {
    return dbEffect(() => db.select().from(userFavorite).where(eq(userFavorite.userId, userId)).orderBy(asc(userFavorite.order)));
}

export function addFavorite(params: { id: string; userId: string; chunkId: string; order: number }) {
    return dbEffect(async () => {
        const [created] = await db.insert(userFavorite).values(params).onConflictDoNothing().returning();
        return created ?? null;
    });
}
