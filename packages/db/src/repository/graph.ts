...
export function getAllChunksMeta(userId?: string, codebaseId?: string, workspaceId?: string) {
    return dbEffect(() => {
        ...
        if (workspaceId) {
            ...
            const inSpaces = db.select({ chunkId: chunkSpace.chunkId }).from(chunkSpace).where(inArray(chunkSpace.spaceId, inWorkspace));
            const inAnySpace = db.select({ chunkId: chunkSpace.chunkId }).from(chunkSpace);
            conditions.push(or(sql`${chunk.id} IN (${inSpaces})`, sql`${chunk.id} NOT IN (${inAnySpace})`)!);
        } else if (codebaseId) {
            const inSpace = db.select({ chunkId: chunkSpace.chunkId }).from(chunkSpace).where(eq(chunkSpace.spaceId, codebaseId));
            const inAnySpace = db.select({ chunkId: chunkSpace.chunkId }).from(chunkSpace);
            conditions.push(or(sql`${chunk.id} IN (${inSpace})`, sql`${chunk.id} NOT IN (${inAnySpace})`)!);
        }
        const query = db
            .select({
                id: chunk.id,
