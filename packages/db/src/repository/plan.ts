...
export function listPlansWithRollups(filter: ListPlansFilter): Effect.Effect<PlanListRow[], DatabaseError> {
    return dbEffect(async () => {
        ...
        return rows.map(r => ({
            ...
            lastActivityAt: r.lastTaskUpdate && r.lastTaskUpdate > r.plan.updatedAt ? r.lastTaskUpdate : r.plan.updatedAt
        }));
    });
}
...
export function duplicatePlan(sourceId: string, userId: string): Effect.Effect<Plan, DatabaseError> {
    return dbEffect(async () => {
        return await db.transaction(async tx => {
            ...
            const reqs = await tx.select().from(planRequirement).where(eq(planRequirement.planId, sourceId));
            if (reqs.length > 0) {
                await tx
                    .insert(planRequirement)
                    .values(reqs.map(r => ({ planId: newPlanId, requirementId: r.requirementId, order: r.order })));
            }

            // Analyze items.
            const items = await tx.select().from(planAnalyzeItem).where(eq(planAnalyzeItem.planId, sourceId));
            if (items.length > 0) {
            ...
            const sourceTasks = await tx.select().from(planTask).where(eq(planTask.planId, sourceId));
            ...
            if (sourceTasks.length > 0) {
                ...
                const links = await tx.select().from(planTaskChunk).where(inArray(planTaskChunk.taskId, sourceTaskIds));
                if (links.length > 0) {
                ...
                // Task dependencies.
                const deps = await tx.select().from(planTaskDependency).where(inArray(planTaskDependency.taskId, sourceTaskIds));
                if (deps.length > 0) {
...
export function listPlanRequirements(planId: string): Effect.Effect<PlanRequirement[], DatabaseError> {
    return dbEffect(async () =>
        db.select().from(planRequirement).where(eq(planRequirement.planId, planId)).orderBy(asc(planRequirement.order))
    );
}

export function addPlanRequirement(planId: string, requirementId: string): Effect.Effect<PlanRequirement, DatabaseError> {
    return dbEffect(async () => {
        const existing = await db.select({ order: planRequirement.order }).from(planRequirement).where(eq(planRequirement.planId, planId));
...
export function removePlanRequirement(planId: string, requirementId: string): Effect.Effect<void, DatabaseError> {
    return dbEffect(async () => {
        await db.delete(planRequirement).where(and(eq(planRequirement.planId, planId), eq(planRequirement.requirementId, requirementId)));
    });
}
...
export function listTasks(planId: string): Effect.Effect<PlanTask[], DatabaseError> {
    return dbEffect(async () => db.select().from(planTask).where(eq(planTask.planId, planId)).orderBy(asc(planTask.order)));
}

export function createTask(input: NewPlanTask): Effect.Effect<PlanTask, DatabaseError> {
    return dbEffect(async () => {
        const existing = await db.select({ order: planTask.order }).from(planTask).where(eq(planTask.planId, input.planId));
        const maxOrder = existing.reduce((m, r) => Math.max(m, r.order), -1);
        const [row] = await db
            .insert(planTask)
            .values({ ...input, order: maxOrder + 1 })
...
import type { NewPlanExternalLink, NewPlanTaskExternalLink, PlanExternalLink, PlanTaskExternalLink } from "../schema/plan";
