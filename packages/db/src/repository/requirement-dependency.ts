...
export function addDependency(requirementId: string, dependsOnId: string) {
    return dbEffect(async () => {
        const result = await db.insert(requirementDependency).values({ requirementId, dependsOnId }).onConflictDoNothing().returning();
...
export function getTransitiveDependencies(requirementId: string) {
    return dbEffect(async () => {
        ...
        if (await isAgeAvailable()) {
            ...
            const [ancestors, descendants] = await Promise.all([fetchById(ancestorIds), fetchById(descendantIds)]);
        ...
        const edgeResult = await db
            ...
            .where(
                inArray(
                    requirementDependency.requirementId,
                    [requirementId, ...ancestorIds, ...descendantIds].length > 0
