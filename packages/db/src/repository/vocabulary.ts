...
export function updateVocabularyEntry(id: string, params: { word?: string; category?: string; expects?: string[] }) {
    return dbEffect(async () => {
...
export function deleteVocabularyEntry(id: string) {
    return dbEffect(async () => {
        const [deleted] = await db.delete(vocabularyEntry).where(eq(vocabularyEntry.id, id)).returning();
        return deleted ?? null;
    });
}
...
export function getVocabularyEntry(id: string) {
    return dbEffect(async () => {
        const [found] = await db.select().from(vocabularyEntry).where(eq(vocabularyEntry.id, id));
        return found ?? null;
    });
}

const STANDARD_MODIFIERS = ["a", "an", "the", "is", "are", "was", "were", "with", "on", "to", "their", "not", "has", "have", "they", "it"];
