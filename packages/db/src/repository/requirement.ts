...
export function getRequirementsByIds(ids: string[], userId: string) {
    return dbEffect(() =>
        db
            .select({ id: requirement.id, useCaseId: requirement.useCaseId, title: requirement.title, status: requirement.status })
            .from(requirement)
            .where(and(inArray(requirement.id, ids), eq(requirement.userId, userId)))
    );
}
...
export function setRequirementOrder(requirementIds: string[]) {
    return dbEffect(async () => {
        for (let i = 0; i < requirementIds.length; i++) {
            ...
            if (!reqId) continue;
            await db.update(requirement).set({ order: i }).where(eq(requirement.id, reqId));
        }
