...
async function insertRules(
    ctx: SeedContext,
    matrixId: string,
    rules: Array<{ title: string; category: string; key: string; description?: string }>
) {
    for (let i = 0; i < rules.length; i++) {
        ...
        await ctx.db
            .insert(behaviorRule)
            .values({ id, matrixId, title: rule.title, description: rule.description, category: rule.category, order: i });
...
export async function seed(ctx: SeedContext): Promise<void> {
    ...
    await ctx.db.insert(behaviorMatrix).values({
        ...
        layer: "invariant",
        description:
            "Rules that must always hold across domain entities — data integrity, ownership, lifecycle, and structural constraints",
        spaceId,
        userId: ctx.userId
    });
    ...
    totalRules += await insertRules(ctx, invariantId, [
        ...
        {
            title: "BDD steps follow given→when→then order",
            category: "Validation",
            key: "r-bdd-order",
        ...
        {
            title: "Builtin entries are read-only",
            category: "Ownership",
            key: "r-builtin-readonly",
            description: "Builtin templates, chunk types, and connection relations cannot be modified or deleted"
        },

        // --- Lifecycle ---
        {
            title: "Cascade deletes to children",
            category: "Lifecycle",
            key: "r-cascade-delete",
        ...
        {
            title: "Status transitions are ungated",
            category: "Lifecycle",
            key: "r-status-ungated",
        ...
        // --- Data Integrity ---
        {
            title: "Optional codebase scoping",
            category: "Data Integrity",
            key: "r-codebase-scope",
        ...
        {
            title: "Feature deltas are sparse JSONB",
            category: "Data Integrity",
            key: "r-delta-sparse",
    ...
    totalRules += await insertRules(ctx, contractId, [
        ...
        {
            title: "Requirement AI generation",
