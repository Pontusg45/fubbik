...
export const cleanupCommand = new Command("cleanup")
    ...
    .action(async (opts, cmd) => {
        ...
        try {
            ...
            const filtered = opts.type ? chunks.filter((c: any) => c.type === opts.type) : chunks;
            ...
            for (const chunk of filtered) {
                ...
                if (chunk.type === "guide" && tags.some((tag: string) => planFilePattern.test(tag) && tag.includes(".md"))) {
                    candidates.push({
                        id: chunk.id,
                        title: chunk.title,
                        type: chunk.type,
            ...
            for (const [, dupes] of titleMap) {
                if (dupes.length > 1) {
                    ...
                    const sorted = dupes.sort((a: any, b: any) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
            ...
            if (!opts.confirm) {
                ...
                if (planArtifacts.length > 0) {
                    console.error(`  ${planArtifacts.length} plan task artifacts (type: guide, tagged with plan filenames)`);
                }
                if (nearEmpty.length > 0) {
                    console.error(`  ${nearEmpty.length} near-empty chunks (< 50 chars)`);
                }
                if (duplicates.length > 0) {
                    console.error(`  ${duplicates.length} duplicate titles`);
                }
                console.error(`\nTotal: ${candidates.length} chunks flagged`);
                console.error(formatDim("Run 'fubbik cleanup --confirm' to remove."));
                return;
            }
            ...
            for (const candidate of candidates) {
                try {
                    const delRes = await fetch(`${serverUrl}/api/chunks/${candidate.id}`, { method: "DELETE" });
                    if (delRes.ok) {
                        console.error(formatSuccess(`Removed: ${candidate.title} ${formatDim(`(${candidate.reason})`)}`));
                        removed++;
                    } else {
                        console.error(formatError(`Failed: ${candidate.title} (${delRes.statusText})`));
                        failed++;
                    }
                } catch {
                    console.error(formatError(`Failed: ${candidate.title} (network error)`));
                    failed++;
                }
            }
