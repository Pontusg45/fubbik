...
export function showChunkDetail(api: FubbikApi, chunkId: string) {
    const panel = vscode.window.createWebviewPanel("fubbik.chunkDetail", "Loading...", vscode.ViewColumn.One, { enableScripts: true });

    const nonce = getNonce();

    // Show loading state
    panel.webview.html = getBaseHtml(panel.webview, nonce, `<p class="muted">Loading chunk...</p>`, "");

    // Fetch and render
    api.getChunk(chunkId)
        .then(({ chunk }) => {
