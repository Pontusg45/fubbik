...
function SpacesPage() {
    ...
    return (
        <PageContainer>
            <PageHeader icon={FolderGit2} title="Spaces" count={spaces.length} />
                            ...
                            {spaces.map(c => (
                                <div key={c.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                                    <div className="min-w-0 flex-1">
                                        ...
                                        {c.remoteUrl && <p className="text-muted-foreground truncate text-sm">{c.remoteUrl}</p>}
                                        {"localPaths" in c && Array.isArray(c.localPaths) && c.localPaths.length > 0 && (
                                            <p className="text-muted-foreground text-xs">{c.localPaths.join(", ")}</p>
                                        )}
                                    </div>
