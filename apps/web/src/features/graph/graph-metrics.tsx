...
export function GraphMetrics({ nodes, edges, onNodeClick }: GraphMetricsProps) {
	...
	const metrics = useMemo(() => {
		...
		const totalConnections = [...counts.values()].reduce(
			(a, b) => a + b,
			0,
		);
		const avgConnections =
			chunkNodes.length > 0 ? totalConnections / chunkNodes.length : 0;
		const maxEdges =
			(chunkNodes.length * (chunkNodes.length - 1)) / 2;
	...
	return (
		<div className="absolute bottom-4 left-4 z-10 max-w-[240px] rounded-lg border bg-background/90 backdrop-blur-sm">
			<button
				...
				<div className="space-y-3 border-t px-3 py-2">
					...
					<div className="grid grid-cols-2 gap-2 text-xs">
						...
						<div>
							...
							<p className="font-medium">
								{metrics.avgConnections.toFixed(1)}
							</p>
						</div>
						<div>
							<p className="text-[10px] text-muted-foreground">
								...
								{metrics.hubs.map(([id, count]) => {
									...
									return (
										<button
											...
											className="flex w-full items-center justify-between rounded px-1.5 py-0.5 text-[11px] hover:bg-muted"
											type="button"
										>
					...
					{metrics.isolated.length > 0 && (
						<div>
							<p className="mb-1 flex items-center gap-1 text-[10px] font-medium uppercase text-muted-foreground">
								<Unlink className="size-2.5" /> Isolated (
								{metrics.isolated.length})
							</p>
									...
									.map((node) => {
										...
										return (
											<button
												...
												className="block w-full truncate rounded px-1.5 py-0.5 text-left text-[11px] hover:bg-muted"
												type="button"
											>
												{label}
											</button>
					...
					<div>
						...
						<div className="flex flex-wrap gap-1">
							{[...metrics.typeCounts.entries()].map(
								([type, count]) => (
									<Badge
										key={type}
										variant="secondary"
