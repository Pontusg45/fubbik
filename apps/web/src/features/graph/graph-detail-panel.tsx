...
export function GraphDetailPanel({
    ...
    return (
        <div className="bg-background flex h-full flex-col md:border-r">
            {/* Header */}
            <div className="flex items-center justify-between border-b px-4 py-3">
                <span className="text-muted-foreground text-xs font-medium">Chunk Details</span>
                                ...
                                {connections.length > 0 && (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            {outgoing.length > 0 && (
                                                <div className="mb-3">
                                                    <p className="text-muted-foreground mb-1 text-[10px] font-medium uppercase">Links to</p>
                                                    <div className="space-y-1">
                                                        {outgoing.map(conn => (
                                                            <button
                                                                key={conn.id}
                                                                onClick={() => onNavigateToChunk(conn.targetId)}
                                                                className="hover:bg-muted flex w-full items-center justify-between rounded-md border px-2.5 py-1.5 text-left text-xs transition-colors"
                                                            >
                                                                <span className="truncate font-medium">
                                                                    {conn.title ?? conn.targetId.slice(0, 8)}
                                                                </span>
                                                                <Badge
                                                                    variant="outline"
                                                                    size="sm"
                                                                    ...
                                                                    style={{
                                                                        borderColor: relationColor(conn.relation),
                                                ...
                                                <div>
                                                    ...
                                                    <div className="space-y-1">
                                                        {incoming.map(conn => (
                                                            ...
                                                            >
                                                                <span className="truncate font-medium">
                                                                    {conn.title ?? conn.sourceId.slice(0, 8)}
                                                                </span>
                                                                <Badge
                                                                    variant="outline"
                                                                    size="sm"
