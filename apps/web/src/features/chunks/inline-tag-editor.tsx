...
export function InlineTagEditor({ tags, onUpdate, loading }: InlineTagEditorProps) {
    ...
    const filteredSuggestions = (allTags ?? []).filter(
        t => t.name.toLowerCase().includes(input.toLowerCase()) && !tagNames.includes(t.name) && input.trim().length > 0
    );
        ...
        <div className="flex flex-wrap items-center gap-1.5">
            {tags.map(tag => (
                <Badge key={tag.id} variant="secondary" className="gap-1">
                    {tag.name}
                    <button onClick={() => removeTag(tag.name)} className="hover:text-destructive ml-0.5" disabled={loading}>
                        <X className="h-3 w-3" />
                    </button>
                </Badge>
            ))}
                ...
                <div className="relative">
                    <Input
                        ...
                        onKeyDown={e => {
                            if (e.key === "ArrowDown") {
                                e.preventDefault();
                                setHighlightedIndex(i => (i < filteredSuggestions.length - 1 ? i + 1 : i));
                        ...
                        >
                            {filteredSuggestions.slice(0, 10).map((s, i) => (
                                <button
                                    ...
                                    className={`text-popover-foreground hover:bg-accent w-full px-2 py-1 text-left text-xs ${
