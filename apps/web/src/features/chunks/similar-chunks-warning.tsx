...
export function SimilarChunksWarning({ title, content, excludeId }: SimilarChunksWarningProps) {
    ...
    const { data: similarChunks } = useQuery({
        ...
        queryFn: async () => {
            const { data, error } = await api.api.chunks["check-similar"].post({
                title: debouncedTitle,
                content: debouncedContent,
                ...(excludeId ? { excludeId } : {})
            });
            if (error) throw new Error("Failed to check similar chunks");
            return data;
        },
    ...
    return (
        <div className="rounded-md border border-yellow-500/30 bg-yellow-500/10 px-3 py-2">
            <p className="text-xs font-medium text-yellow-600 dark:text-yellow-400">Semantically similar chunks found:</p>
            <ul className="mt-1 space-y-0.5">
                {similarChunks.map(c => (
                    <li key={c.id} className="flex items-center justify-between text-xs">
                        <Link to="/chunks/$chunkId" params={{ chunkId: c.id }} className="text-yellow-600 underline dark:text-yellow-400">
                            {c.title}
                        </Link>
                        <span className="text-muted-foreground ml-2">{Math.round(c.similarity * 100)}% match</span>
                    </li>
                ))}
            </ul>
        </div>
