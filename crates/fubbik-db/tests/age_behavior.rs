...
async fn upsert_behavior_rule_is_idempotent_and_updates_props(pool: sqlx::PgPool) {
    ...
    age::upsert_behavior_rule(&pool, &rule("r1", "First"))
        .await
        .unwrap();
    age::upsert_behavior_rule(&pool, &rule("r1", "Renamed"))
        .await
        .unwrap();
...
async fn behavior_rule_title_survives_quotes_and_apostrophes(pool: sqlx::PgPool) {
    ...
    assert_eq!(
        rules.len(),
        1,
        "an apostrophe must not break the Cypher literal"
    );
...
async fn governs_edges_rebuild_without_duplicating(pool: sqlx::PgPool) {
    ...
    age::cypher(&pool, "CREATE (:code_file {id: 'src/auth/session.ts'})")
        .await
        .unwrap();
    age::upsert_behavior_rule(&pool, &rule("r1", "Sessions expire"))
        .await
        .unwrap();

    for _ in 0..2 {
        age::delete_governs_edges(&pool, "r1").await.unwrap();
        age::link_governs(&pool, "r1", "file", "auth/session.ts")
            .await
            .unwrap();
    }
    ...
    assert_eq!(
        edges.len(),
        1,
        "delete-then-relink must not accumulate edges"
    );
