-- The 0001 baseline is a schema-only pg_dump of the live TypeScript app — it
-- carries no data. `chunk.type` is a FK to `chunk_type(id)`, `chunk_connection.relation`
-- is a FK to `connection_relation(id)`, and `space.kind` is a FK to `space_kind(id)`
-- (all ON DELETE RESTRICT), so without these built-in rows chunks, connections, and
-- spaces can't be created at all.
--
-- Mirrors the TypeScript app's own seed migrations so both stacks agree on the same
-- built-in vocabulary:
--   packages/db/src/migrations/0001_seed_builtin_catalogs.sql (chunk_type, connection_relation)
--   packages/db/src/migrations/0004_codebase_to_space.sql (space_kind)
--
-- Deliberately excludes tag_type: it has rows in the live DB but is user-managed
