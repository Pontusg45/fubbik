...
**Architecture:** Repositories return `Effect<T, DatabaseError>`. Services compose Effects and add `NotFoundError`/`AuthError`. Routes call `Effect.runPromise` — thrown errors caught by a global Elysia `.onError` handler that maps `_tag` to HTTP status codes. Zero try-catch blocks remain.
...
In `packages/api/package.json`, remove `"drizzle-orm": "^0.45.1"` from dependencies (repositories in db package handle all drizzle calls now).
...
Expected: All tests pass (health test uses HTTP boundary, not repository directly)
...
Note: The health route uses `Effect.match` to handle both success and failure locally — it doesn't throw to the global error handler because a DB-down health check is a 200 with degraded status, not an error. However, the original returned 503 for degraded. Let's preserve that:
