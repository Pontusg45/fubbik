...
**Architecture:** Bottom-up: create repositories in db first, then split api into feature folders with services calling repositories, then reorganize web features, finally CLI. Each task preserves existing behavior — no new features, pure refactor. Tests must pass after every task.
...
Extract all chunk DB queries from `packages/api/src/index.ts` into pure functions. Each function takes typed params, returns typed results. No Elysia types.
...
export async function updateChunk(chunkId: string, userId: string, body: { title?: string; content?: string; type?: string; tags?: string[] }) {
...
export const chunkRoutes = new Elysia()
  ...
  .get("/chunks/:id", async ({ session, set, params }) => {
    ...
    try {
      ...
      if (!result) { set.status = 404; return { message: "Chunk not found" }; }
      return result;
    } catch (err) {
  ...
  .patch(
    ...
    async ({ session, set, params, body }) => {
      ...
      try {
        ...
        if (!updated) { set.status = 404; return { message: "Chunk not found" }; }
        return updated;
      } catch (err) {
  ...
  .delete("/chunks/:id", async ({ session, set, params }) => {
    ...
    try {
      ...
      if (!deleted) { set.status = 404; return { message: "Chunk not found" }; }
...
Expected: All 10 tests pass (8 API + 2 schema)
...
These are placeholders. As chunk-specific and dashboard-specific components are built, they go here instead of the flat `components/` folder.
...
Expected: type-check, lint, test, build, fmt:check, sherif all pass
