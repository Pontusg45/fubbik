...
A phased plan covering human UX, AI agent integration, and shared improvements across fubbik. Organized into 6 phases by dependency and
impact.
...
Each feature within a phase gets its own spec → plan → implementation cycle. Features within a phase can be parallelized if they touch
different files.
