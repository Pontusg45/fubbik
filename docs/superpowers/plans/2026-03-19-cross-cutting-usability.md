...
**Goal:** Implement cross-cutting usability improvements: offline indicator, consistent empty states, and export enhancements — features
that span multiple surfaces.

**Architecture:** Offline indicator is a web UI component using navigator.onLine + server health polling. Export improvements add new API
query params and UI options. These are small, focused changes.
...
Run: `cd apps/web && pnpm vitest run src/__tests__/connection-status.test.tsx` Expected: FAIL
...
Run: `cd apps/web && pnpm vitest run src/__tests__/connection-status.test.tsx` Expected: PASS
...
In `packages/api/src/chunks/routes.ts`, modify the GET `/chunks/export` handler. **Important:** Add an Elysia query schema to validate the
`format` param:
