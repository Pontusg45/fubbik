...
**Goal:** Improve the VS Code extension from a basic sidebar viewer to a productive in-editor knowledge tool with filtering, editing,
file-aware surfacing, more commands, and a status bar indicator.

**Architecture:** All changes in `apps/vscode/src/`. The extension uses VS Code's webview API for the sidebar and panels, and communicates
with the fubbik server via HTTP (FubbikApi class). No backend changes needed — all endpoints already exist.
...
const [chunks, tags] = await Promise.all([this.api.getChunks(this.codebaseId), this.api.getTags()]);
...
export function showEditChunk(api: FubbikApi, chunk: Chunk, onUpdated: () => void) {
    const panel = vscode.window.createWebviewPanel("fubbikEditChunk", `Edit: ${chunk.title}`, vscode.ViewColumn.One, {
        enableScripts: true
    });
...
function buildEditForm(chunk: Chunk): string {
    ...
    <label>Type</label>
    <select id="type">
      ${["note", "document", "reference", "schema", "checklist"]
          .map(t => `<option value="${t}" ${t === chunk.source ? "selected" : ""}>${t}</option>`)
          .join("")}
    </select>
...
${fileChunks.map(c => `
<div class="list-item highlighted" data-id="${c.id}">
    ...`).join("")}
    <hr />
    ` : ""}
</div>
