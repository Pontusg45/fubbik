...
**Architecture:** Recommendations use session co-reference patterns from `session_chunk_ref`. Search operators parse a structured query
syntax client-side. Bookmarks use a new `chunk_bookmark` table.
...
- [ ] **Step 3:** On chunk detail page, add a "Frequently Referenced With" section (inside a CollapsibleSection, default closed) showing
      recommended chunks.
...
- [ ] **Step 3:** In command palette, detect operators and pass as separate query params. Show a hint: "Operators: type:note tag:auth
      updated:>7d".
...
- [ ] **Step 3:** On chunk detail, add a bookmark button (different from favorite/pin — bookmarks have notes). Show a small modal for the
      note when bookmarking.
