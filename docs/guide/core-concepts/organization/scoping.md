...
Optionally register expected scope keys at `/settings` to get autocomplete and validation in the UI. The `scope_key` table defines keys with
types (string, number, boolean, enum) and allowed values. Scope remains free-form — the registry is opt-in guidance.
