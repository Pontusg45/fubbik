...
- **Purpose**: Local-first knowledge framework for storing, navigating, and evolving structured knowledge about codebases. Designed for both
  humans (web UI, graph visualization) and machines (CLI, API, VS Code extension, MCP server).
...
Chunks can be organized per-space. A space is identified by a `kind` discriminator (`code | wiki | notes | research`) and — for `code` kind spaces — a git remote URL (normalized) or local paths stored in the `space_code_metadata` side-table. Chunks can belong to multiple spaces or none (global).

- `space` table with `remoteUrl` (deprecated, now in `space_code_metadata`), `localPaths`, `name`, `kind`
- `space_code_metadata` side-table holding `remoteUrl`, `localPaths`, `defaultBranch` for `kind = 'code'` spaces
- `chunk_space` join table (many-to-many; formerly `chunk_codebase`)
- `workspace` table groups related spaces (e.g., frontend + backend + infra)
- `workspace_space` join table (formerly `workspace_codebase`)
- `feature_space` join table (formerly `feature_codebase`)
- CLI auto-detects space via git remote
- Web UI has a space/workspace switcher in the nav

### Connections

Directed edges between chunks: `sourceId → targetId` with a `relation` type (related_to, part_of, depends_on, extends, references, supports,
contradicts, alternative_to). Connections are global (not space-scoped), enabling cross-project knowledge linking.

### Templates

Built-in + user-created chunk templates (Convention, Architecture Decision, Runbook, API Endpoint). Built-in templates are seeded via SQL
migration and are read-only. Users can duplicate and customize.
...
- `plan` table: `title`, `description` (markdown), `status` (`draft | analyzing | ready | in_progress | completed | archived` — labels only,
  ungated), `userId`, `spaceId`, `completedAt`
...
- `requirement` table with `title`, `description`, `steps` (JSONB array of `{keyword, text}`), `status` (passing/failing/untested),
...
- **Connectivity** (0-25): number of connections Exposed via chunk detail API response and shown as a badge on the detail page.
...
- `feature_space` join table (optional space association; formerly `feature_codebase`)
- `chunk_feature_delta` table: one delta per chunk per feature, contains JSONB with changed content fields only (title, content, type,
  rationale, alternatives, consequences, summary)
...
- Feature switcher in nav bar alongside space switcher
...
- `staleness_scan` table tracks per-space last scanned git commit SHA for incremental scans
...
Optional registry for chunk scope keys. The `scope_key` table defines expected keys with types (string, number, boolean, enum) and allowed
values. Used for UI autocomplete and optional validation. Scope remains free-form JSONB — the registry is opt-in.
...
- **Services** (`packages/api/src/*/service.ts`): Compose repository Effects, add business logic, introduce
...
- **Global error handler** (`packages/api/src/index.ts`): Extracts Effect errors from FiberFailure, maps `_tag` to HTTP status codes
  (ValidationError->400, AuthError->401, NotFoundError->404, DatabaseError->500).

### Frontend: Feature-based Structure

- Route files in `apps/web/src/routes/`
- Feature components in `apps/web/src/features/` (e.g., `features/auth/`, `features/graph/`, `features/spaces/`, `features/plans/`,
  `features/chunks/`)
...
- **Enrichment** (`enrichChunks`): Fetches full rows, connections, tags, stale flags, proposals, active feature overlays. Computes health
  scores.
...
- Tools organized by domain: `tools.ts` (core), `session-tools.ts`, `plan-tools.ts`, `requirement-tools.ts`, `suggestion-tools.ts`,
  `context-tools.ts`
- Each file exports `registerXTools(server: McpServer)` function
- Uses `apiFetch` helper for server communication
- Key tools: `create_plan`, `create_plan_from_requirements`, `import_plan_markdown`, `begin_implementation`, `mark_plan_step`,
  `sync_claude_md`
...
- `GET /api/chunks` — list (supports `spaceId`, `workspaceId`, `global`, `allSpaces`, `search`, `type`, `tags`, `tagMode=any|all`,
  `sort`, etc.)
- `POST /api/chunks` — create (with optional `spaceIds`, `rationale`, `alternatives`, `consequences`)
- `GET /api/chunks/:id` — detail (includes connections, spaces, appliesTo, fileReferences, healthScore)
...
- `GET /api/chunks/search/federated` — cross-space search with space names
...
- `GET /api/spaces/detect?remoteUrl=&localPath=` — detect space from git remote or path
- `PATCH /api/spaces/:id` — update
- `DELETE /api/spaces/:id` — delete

> `/api/codebases` is a deprecated alias kept for backward compatibility with older VS Code extension builds and external consumers.

### Workspaces

- `GET /api/workspaces` — list workspaces
- `POST /api/workspaces` — create
- `GET /api/workspaces/:id` — detail with spaces
- `PATCH /api/workspaces/:id` — update
- `DELETE /api/workspaces/:id` — delete
- `POST /api/workspaces/:id/spaces` — add space
- `DELETE /api/workspaces/:id/spaces/:spaceId` — remove space
...
- `GET /api/plans` — list (filters: `spaceId`, `status`, `requirementId`, `includeArchived`)
- `POST /api/plans` — create (body: `title`, `description?`, `spaceId?`, `requirementIds?`, `tasks?`)
- `GET /api/plans/:id` — detail (plan + requirements + analyze grouped by kind + tasks with chunks + dependencies)
- `PATCH /api/plans/:id` — update title/description/status/spaceId
- `DELETE /api/plans/:id`
...
- `GET /api/requirements` — list (supports filters: status, priority, spaceId, search)
...
- `GET /api/context/for-file?path=<path>&spaceId=<id>&deps=<csv>&format=<fmt>&maxTokens=<n>` — chunks relevant to a file (five
  strategies: file-ref, applies-to, dependency, semantic, connected). Formats: `structured-md` (default), `structured-json`, `json-legacy`
- `GET /api/context/for-plan?planId=<id>&maxTokens=<n>&format=<fmt>` — chunks linked to a plan
- `GET /api/context/about?q=<concept>&maxTokens=<n>&spaceId=<id>&format=<fmt>` — semantic + text search for a concept
- `GET /api/context/for-files?paths=<csv>&maxTokens=<n>&spaceId=<id>&format=<fmt>` — chunks for multiple files
...
- `GET /api/chunks/stale` — list undismissed staleness flags (supports `reason`, `spaceId`, `limit`)
- `GET /api/chunks/stale/count` — count of undismissed flags (supports `spaceId`)
...
- `POST /api/chunks/stale/scan-age` — trigger age-based staleness scan (body: `spaceId?`, `thresholdDays?`)

### Features

- `GET /api/features` — list (filters: `spaceId`, `status`, `search`), includes delta count
- `POST /api/features` — create (body: `name`, `description?`, `priority?`, `color?`, `spaceIds?`)
- `GET /api/features/:id` — detail with spaces and deltas
...
- `GET /api/graph` — graph data (nodes + edges + tags, supports `spaceId`, `workspaceId`)
...
- `/dashboard` — overview with clickable stats, recent chunks, favorites, recently viewed, health summary, "Attention Needed" staleness
  widget
...
- `/chunks/:id` — chunk detail with collapsible sections, inline tag editor, health badge, connection arrows, dependency tree (grouped by
  relation type), related chunk suggestions (embedding-based), staleness banners, feature overlay indicators
...
- `/graph` — knowledge graph visualization (force-directed, hierarchical, radial layouts; tag grouping with clickable legend filters; path
  finding; workspace view with cross-space edge styling; focus mode via double-click; saveable filter presets)
...
- `/workspaces` — workspace management (group spaces)
...
- `/plans/new` — simple form (title + description + optional space)
- `/plans/:id` — sticky header + four sections (Description, Requirements, Analyze, Tasks)
- `/import` — dedicated markdown docs import with folder upload, preview table, space selection
- `/login` — authentication
- `/settings` — user/space/instance settings
...
- `fubbik space add/list/remove/current` — space management
- `fubbik codebase` — deprecated alias of `fubbik space`; prints a warning when used
...
- `fubbik quick "title"` — one-liner chunk creation (auto-detects space, supports `--type`, `--tags`, pipe stdin for content)
...
- `fubbik import <path>` — import chunks from JSON file, .md file, or directory (use `--server`/`--space` for server mode; `--codebase` is a deprecated alias)
- `fubbik completions <shell>` — generate shell completions (zsh)
- List/add/search support `--global` and `--space <name>` flags for scoping (`--codebase` is a deprecated alias)
...
- 1 space (fubbik, kind=code), 9 applies-to patterns, 5 file references
...
- Ollama failures are handled four different ways, all matching the original TypeScript:
  ...
  and `suggest_vocabulary` is infallible — no probe, no error path, every failure mode
  collapses to `[]`.
...
Configure `fubbik.serverUrl` (default: `http://localhost:3000`) and `fubbik.webAppUrl` (default: `http://localhost:3001`) in VS Code
settings.
